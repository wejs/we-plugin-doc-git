  ...
  mkdirp (we.config.doc.folder, function (err) {
    ...
    async.each(we.config.doc.projects, function (project, next) {
      fs.stat(we.config.doc.folder + '/' + project.name, function (err) {
        if (err) {
          if (err.code == 'ENOENT') {
            if (!we.config.doc.cloneInBootstrap) return done();
        ...
        if (!we.config.doc.pullInBootstrap) return done();

        var repo = git(we.config.doc.folder + '/' + project.name);
        repo.pull('origin', 'master',function (err, x) {
          we.log.warn('upadate', err, x)
          return next();
        });
      });
    }, function(err) {
